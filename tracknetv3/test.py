        ...
        for f in range(seq_len):
            ...
            if d_i != prev_d_i:
                if c_true is not None and c_pred is not None:
                    ...
                    cx_true, cy_true = int(bbox_true[0] + bbox_true[2] / 2), int(
                        bbox_true[1] + bbox_true[3] / 2
                    )
...
def generate_inpaint_mask(pred_dict, th_h=30):
    ...
    while j < len(vis_pred):
        ...
        if j == i:
            ...
            inpaint_mask[:j] = 1
        elif (i > 1 and y[i - 1] > threshold) and (
            j < len(vis_pred) and y[j] > threshold
        ):
            inpaint_mask[i:j] = 1
        else:
...
def linear_interp(target, inpaint_mask):
    assert len(target) == len(
        inpaint_mask
    ), "Length of target and inpaint_mask should be the same"
...
def get_eval_res(pred_dict):
    ...
    for pred_type in pred_types:
        res[pred_types_map[pred_type]] += int(
            (type_res == pred_types_map[pred_type]).sum()
        )
...
def eval_tracknet(model, data_loader, param_dict):
    ...
    for step, (i, x, y, _, _) in enumerate(data_prob):
        ...
        pred_dict = evaluate(
            i, y_true=y, y_pred=y_pred, tolerance=param_dict["tolerance"]
        )
...
def eval_inpaintnet(model, data_loader, param_dict):
    ...
    losses = []
    confusion_matrix = {
        eval_type: np.zeros(5) for eval_type in inpaintnet_eval_types
    }  # TP, TN, FP1, FP2, FN
    if param_dict["verbose"]:
        data_prob = tqdm(data_loader)
    else:
    ...
    for step, (i, coor_pred, coor, _, _, inpaint_mask) in enumerate(data_prob):
        coor_pred, coor, inpaint_mask = (
            coor_pred.float().cuda(),
            coor.float().cuda(),
            inpaint_mask.float().cuda(),
        )
        ...
        with torch.no_grad():
            ...
            # Thresholding
            th_mask = (coor_inpaint[:, :, 0] < COOR_TH) & (
        ...
        for eval_type in inpaintnet_eval_types:
            if eval_type == "inpaint":
                ...
                pred_dict = evaluate(
                    i, c_true=coor, c_pred=coor_pred, tolerance=param_dict["tolerance"]
                )
            else:
...
def test(model, split, param_dict, save_inpaint_mask=False, linear_interp=False):
    ...
    for rally_dir in rally_dirs:
        ...
        if linear_interp:
            ...
            tmp_pred = test_rally(
                model, rally_dir, param_dict, save_inpaint_mask=save_inpaint_mask
            )
        ...
        if save_inpaint_mask:
            ...
            write_pred_csv(
                tmp_pred, save_file=csv_file, save_inpaint_mask=save_inpaint_mask
            )
    ...
    if inpaintnet is None:
        ...
        if param_dict["eval_mode"] == "nonoverlap":
            ...
            for step, (i, x, y, _, _) in enumerate(data_prob):
                ...
                ensemble_y_pred = torch.empty(
                    (0, 1, HEIGHT, WIDTH), dtype=torch.float32
                )
                ...
                for b in range(b_size):
                    ...
                    ensemble_i = torch.cat(
                        (ensemble_i, i[b][0].reshape(1, 1, 2)), dim=0
                    )
                    ensemble_y = torch.cat(
                        (ensemble_y, y[b][0].reshape(1, 1, HEIGHT, WIDTH)), dim=0
                    )
                    ...
                    if sample_count == num_sample:
                        ...
                        for f in range(1, seq_len):
                            ...
                            ensemble_y = torch.cat(
                                (ensemble_y, y[-1][f].reshape(1, 1, HEIGHT, WIDTH)),
                                dim=0,
                            )
                            ensemble_y_pred = torch.cat(
        ...
        if param_dict["eval_mode"] == "nonoverlap":
            ...
            for step, (i, coor_pred, coor, _, _, inpaint_mask) in enumerate(data_prob):
                ...
                with torch.no_grad():
                    coor_inpaint = (
                        inpaintnet(coor_pred.cuda(), inpaint_mask.cuda()).detach().cpu()
                    )
                    coor_inpaint = coor_inpaint * inpaint_mask + coor_pred * (
                        1 - inpaint_mask
                    )  # replace predicted coordinates with inpainted coordinates
            ...
            for step, (i, coor_pred, coor, _, _, inpaint_mask) in enumerate(data_prob):
                ...
                coor_inpaint_buffer = torch.cat(
                    (coor_inpaint_buffer, coor_inpaint), dim=0
                )
                ensemble_i = torch.empty((0, 1, 2), dtype=torch.float32)
                ...
                for b in range(b_size):
                    ...
                    ensemble_coor = torch.cat(
                        (ensemble_coor, coor[b][0].view(1, 1, 2)), dim=0
                    )
                    ...
                    if sample_count == num_sample:
                        ...
                        for f in range(1, seq_len):
                            ...
                            ensemble_i = torch.cat(
                                (ensemble_i, i[b][f].view(1, 1, 2)), dim=0
                            )
                            ensemble_coor = torch.cat(
                                (ensemble_coor, coor[b][f].view(1, 1, 2)), dim=0
                            )
                            ensemble_coor_inpaint = torch.cat(
...
def test_rally_linear(model, rally_dir, param_dict):
            ...
            for b in range(b_size):
                ...
                if sample_count == num_sample:
                    ...
                    for f in range(1, seq_len):
                        ...
                        ensemble_y = torch.cat(
                            (ensemble_y, y[-1][f].reshape(1, 1, HEIGHT, WIDTH)), dim=0
                        )
                        ensemble_y_pred = torch.cat(
                            (ensemble_y_pred, y_pred.reshape(1, 1, HEIGHT, WIDTH)),
                            dim=0,
    ...
    # Linear inpainting
    x_pred = (
        linear_interp(tracknet_pred_dict["X"], tracknet_pred_dict["Inpaint_Mask"])
        / WIDTH
    )
    y_pred = (
        linear_interp(tracknet_pred_dict["Y"], tracknet_pred_dict["Inpaint_Mask"])
        / HEIGHT
    )
    ...
    for i in range(len(label_df)):
        ...
        coor = torch.cat(
            (coor, torch.tensor([[[x_gt[i], y_gt[i]]]], dtype=torch.float32)), dim=0
        )
        coor_inpaint = torch.cat(
            (
                coor_inpaint,
                torch.tensor([[[x_pred[i], y_pred[i]]]], dtype=torch.float32),
            ),
            dim=0,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tracknet_file", type=str, help="file path of the TrackNet model checkpoint"
    )
    parser.add_argument(
        "--inpaintnet_file",
        type=str,
        default="",
        help="file path of the InpaintNet model checkpoint",
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=4,
        help="difference tolerance of center distance between prediction and ground truth in input size",
    )
    parser.add_argument(
        "--eval_mode",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="file path of the video with label (must in dataset directory with same data_dir)",
    )
    parser.add_argument(
        "--output_pred",
        action="store_true",
        default=False,
        help="whether to output detail prediction result for error analysis",
    )
    parser.add_argument(
        "--output_bbox",
        action="store_true",
        default=False,
        help="whether to output coco format bbox prediction result for mAP evaluation",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
    ...
    if args.tracknet_file:
        tracknet_ckpt = torch.load(args.tracknet_file, map_location=torch.device("cpu"))
    ...
    if args.inpaintnet_file:
        inpaintnet_ckpt = torch.load(
            args.inpaintnet_file, map_location=torch.device("cpu")
        )
    ...
    if args.video_file:
        ...
        # Load label
        csv_file = (
            os.path.join(match_dir, "corrected_csv", f"{rally_id}_ball.csv")
        ...
        label_df = (
            pd.read_csv(csv_file, encoding="utf8").sort_values(by="Frame").fillna(0)
        )
        ...
        eval_analysis_file = os.path.join(
            args.save_dir, f"{args.split}_eval_analysis_{args.eval_mode}.json"
        )  # for error analysis interface
        eval_res_file = os.path.join(
            args.save_dir, f"{args.split}_eval_res_{args.eval_mode}.json"
        )
        ...
        pred_dict = test(
            model, args.split, param_dict, linear_interp=args.linear_interp
        )
        if args.split == "test":
        ...
        if args.output_bbox:
            coco_file = os.path.join(
                args.save_dir, f"{args.split}_coco_res_{args.eval_mode}.json"
            )
            if args.split == "test":
