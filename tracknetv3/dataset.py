...
class Shuttlecock_Trajectory_Dataset(Dataset):
    ...
    def __init__(
        ...
        if self.frame_arr is not None:
            ...
            assert (
                self.data_mode == "heatmap"
            ), f"Invalid data_mode: {self.data_mode}, frame_arr only for heatmap mode"
            ...
            assert (
                self.data_mode == "coordinate"
            ), f"Invalid data_mode: {self.data_mode}, pred_dict only for coordinate mode"
            ...
            img_config_file = os.path.join(
                self.root_dir, f"img_config_{self.HEIGHT}x{self.WIDTH}_{self.split}.npz"
            )
    ...
    def _get_rally_dict(self):
        ...
        rally_dict = {
            "i2p": {
                i: os.path.join(self.root_dir, rally_dir)
                for i, rally_dir in enumerate(rally_dirs)
            },
            "p2i": {
                os.path.join(self.root_dir, rally_dir): i
                for i, rally_dir in enumerate(rally_dirs)
            },
        }
        return rally_dict
    ...
    def _gen_input_file(self, file_name):
        ...
        if self.data_mode == "heatmap":
            ...
            for rally_i, rally_dir in tqdm(self.rally_dict["i2p"].items()):
                ...
                frame_file = np.concatenate(
                    (frame_file, data_dict["frame_file"]), axis=0
                )
            ...
            for rally_i, rally_dir in tqdm(self.rally_dict["i2p"].items()):
                ...
                inpaint_mask = np.concatenate(
                    (inpaint_mask, data_dict["inpaint_mask"]), axis=0
                )
        ...
        if self.data_mode == "heatmap":
            ...
            if "test" in rally_dir:
                csv_file = os.path.join(
                    match_dir, "corrected_csv", f"{rally_id}_ball.csv"
                )
            else:
            ...
            f_file = np.array(
                [
                    os.path.join(rally_dir, f"{f_id}.{IMG_FORMAT}")
            ...
            for i in range(0, len(f_file), self.sliding_step):
                ...
                if len(tmp_frames) == self.seq_len:
                    assert (
                        len(tmp_frames) == len(tmp_coor) == len(tmp_vis)
            ...
            pred_csv_file = os.path.join(
                match_dir, "predicted_csv", f"{rally_id}_ball.csv"
            )
            assert os.path.exists(pred_csv_file), f"{pred_csv_file} does not exist."
            pred_df = (
                pd.read_csv(pred_csv_file, encoding="utf8")
    ...
    def _gen_input_from_pred_dict(self):
        ...
        assert (
            len(x_pred) == len(y_pred) == len(vis_pred) == len(inpaint)
    ...
    def __getitem__(self, idx):
        ...
        if self.frame_arr is not None:
            ...
            if self.frame_alpha > 0:
                ...
                if self.bg_mode:
                    file_format_str = os.path.join(
                        "{}", "frame", "{}", "{}." + IMG_FORMAT
                    )
                    match_dir, rally_id, _ = parse.parse(
                        file_format_str, frame_file[0]
                    )  #'{}/frame/{}/{}.png', frame_file[0])
                    median_file = (
                ...
                for i in range(1, self.seq_len):
                    ...
                    if self.bg_mode == "subtract":
                        ...
                        cur_img = np.array(
                            cur_img.resize(size=(self.WIDTH, self.HEIGHT))
                        )
                        ...
                        diff_img = np.array(
                            diff_img.resize(size=(self.WIDTH, self.HEIGHT))
                        )
                        diff_img = diff_img.reshape(1, self.HEIGHT, self.WIDTH)
                        cur_img = np.array(
                            cur_img.resize(size=(self.WIDTH, self.HEIGHT))
                    ...
                    if vis[i] == 0:
                        ...
                        prev_vis == 0
                        or math.sqrt(
                            pow(prev_coor[0] - coor[i][0], 2)
                            + pow(prev_coor[1] - coor[i][1], 2)
                        )
                        < 10
                    ):
                        inter_coor = coor[i]
                    ...
                    heatmaps = np.concatenate(
                        (heatmaps, inter_heatmap, cur_heatmap), axis=0
                    )
                ...
                if self.bg_mode == "concat":
                    ...
                    median_img = np.array(
                        median_img.resize(size=(self.WIDTH, self.HEIGHT))
                    )
...
class Video_IterableDataset(IterableDataset):
    ...
    def __init__(
        ...
        self.w, self.h = int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(
        ...
        if self.bg_mode:
            self.median = (
                ...
                else self.__gen_median__(max_sample_num, video_range)
            )
    ...
    def __gen_median__(self, max_sample_num, video_range):
        ...
        frame_list = []
        print(start_frame, end_frame, sample_step)
