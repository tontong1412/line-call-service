...
def convert_gt_to_coco_json(data_dir, split, drop=False):
    ...
    for rally_dir in rally_dirs:
        ...
        f, x, y, v = (
            label_df["Frame"].values,
            label_df["X"].values,
            label_df["Y"].values,
            label_df["Visibility"].values,
        )
        if split == "test" and drop:
...
def generate_data_frames(video_file):
    ...
    label_df = pd.read_csv(csv_file, encoding="utf8")
    assert os.path.exists(video_file) and os.path.exists(
        csv_file
    ...
    np.savez(
        os.path.join(rally_dir, "median.npz"), median=median
    )  # Must be lossless, do not save as image format
...
def get_match_median(match_dir):
    ...
    np.savez(
        os.path.join(match_dir, "median.npz"), median=median
    )  # Must be lossless, do not save as image format
...
def get_rally_median(video_file):
    ...
    np.savez(
        os.path.join(save_dir, "median.npz"), median=median
    )  # Must be lossless, do not save as image format
